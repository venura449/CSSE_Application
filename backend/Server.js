...
async function createPoolAndEnsure() {
  ...
  try {
    ...
    try {
      await conn.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS role VARCHAR(50) NOT NULL DEFAULT 'Resident'");
    } catch (e) {
      ...
      try {
        const [cols] = await conn.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'role'", [DB_NAME]);
        if (!cols || cols.length === 0) {
          await conn.query("ALTER TABLE users ADD COLUMN role VARCHAR(50) NOT NULL DEFAULT 'Resident'");
        }
      } catch (ee) {
...
app.post('/api/collections/request', authMiddleware, async (req, res) => {
  ...
  if (req.user.role === 'Collector') return res.status(403).json({ error: 'Collectors cannot create collection requests' });
  ...
  try {
    ...
    try {
      ...
      await conn.query('INSERT INTO collection_requests (id, user_id, item_type, preferred_datetime, preferred_time_label, photos, estimated_cost, status, payment_required) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
        id, req.user.id, item_type, preferred_dt, preferred_time_label || null, photos ? JSON.stringify(photos) : null, Number(estimated_cost) || 0, status, payment_required
      ]);
...
app.put('/api/collections/request/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (req.user.role !== 'Authority' && r.user_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
      if (r.status === 'Paid') return res.status(400).json({ error: 'Cannot edit a paid request' });

      const prefDt = preferred_date ? new Date(preferred_date) : null;
      await conn.query('UPDATE collection_requests SET item_type = COALESCE(?, item_type), preferred_datetime = COALESCE(?, preferred_datetime), preferred_time_label = COALESCE(?, preferred_time_label), photos = COALESCE(?, photos), estimated_cost = COALESCE(?, estimated_cost) WHERE id = ?', [
        item_type || null, prefDt, preferred_time_label || null, photos ? JSON.stringify(photos) : null, (estimated_cost !== undefined ? Number(estimated_cost) : null), id
      ]);
...
app.delete('/api/collections/request/:id', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!r) return res.status(404).json({ error: 'Request not found' });
      if (req.user.role !== 'Authority' && r.user_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
      if (r.status === 'Paid') return res.status(400).json({ error: 'Cannot delete a paid request' });
...
app.post('/api/payments', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (req.user.role === 'Resident' && reqRow.user_id !== req.user.id) return res.status(403).json({ error: 'Cannot pay for another user\'s request' });

      // insert payment
      const [presult] = await conn.query('INSERT INTO payments (request_id, user_id, amount, currency, status, details) VALUES (?, ?, ?, ?, ?, ?)', [request_id, reqRow.user_id, Number(amount), currency || 'USD', 'Completed', JSON.stringify({ by: req.user.id })]);
      const paymentId = presult.insertId;

      // update collection_requests to mark as paid and link payment
      await conn.query('UPDATE collection_requests SET status = ?, payment_id = ?, paid_at = ? WHERE id = ?', ['Paid', paymentId, new Date(), request_id]);

      // create schedule entry so it appears on collections schedule
      const scheduleId = `s-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
      const scheduledAt = reqRow.preferred_datetime ? new Date(reqRow.preferred_datetime) : new Date();
      await conn.query('INSERT INTO schedules (id, user_id, type, scheduled_at, time_label, status, location_lat, location_lng, location_label) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [
        scheduleId, reqRow.user_id, `Special: ${reqRow.item_type}`, scheduledAt, reqRow.preferred_time_label || null, 'Scheduled', null, null, null
      ]);
...
app.get('/api/collections', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      if (req.user.role === 'Collector') {
        // collectors see collections assigned to them - in this simple model, we'll return schedules with status 'Assigned' or 'InProgress' or 'Collected'
        const [rows] = await conn.query("SELECT id, user_id, type, scheduled_at, status, location_lat, location_lng, location_label FROM schedules WHERE status IN ('Assigned','InProgress','Collected') ORDER BY scheduled_at ASC");
        return res.json(rows.map(r => ({ id: r.id, user_id: r.user_id, type: r.type, scheduled_at: r.scheduled_at, status: r.status, location: r.location_lat ? { lat: Number(r.location_lat), lng: Number(r.location_lng), label: r.location_label } : null })));
      }
      // residents: return only their schedules; Authority: return all
      if (req.user.role === 'Resident') {
        const [rows] = await conn.query('SELECT id, type, scheduled_at, status, location_lat, location_lng, location_label FROM schedules WHERE user_id = ? ORDER BY scheduled_at ASC', [req.user.id]);
        return res.json(rows.map(r => ({ id: r.id, type: r.type, scheduled_at: r.scheduled_at, status: r.status, location: r.location_lat ? { lat: Number(r.location_lat), lng: Number(r.location_lng), label: r.location_label } : null })));
      }
      // Authority
      const [rows] = await conn.query('SELECT id, user_id, type, scheduled_at, status, location_lat, location_lng, location_label FROM schedules ORDER BY scheduled_at ASC');
      return res.json(rows.map(r => ({ id: r.id, user_id: r.user_id, type: r.type, scheduled_at: r.scheduled_at, status: r.status, location: r.location_lat ? { lat: Number(r.location_lat), lng: Number(r.location_lng), label: r.location_label } : null })));
    } finally { conn.release(); }
  } catch (err) {
    console.error(err);
...
app.get('/api/payments', authMiddleware, async (req, res) => {
  try {
    ...
    try {
      ...
      if (req.user.role === 'Collector') {
        // collectors see payments they've collected - assume payments table has collector_id
        const [rows] = await conn.query('SELECT id, amount, currency, status, created_at FROM payments WHERE collector_id = ? ORDER BY created_at DESC', [req.user.id]);
        return res.json(rows);
      }
      if (req.user.role === 'Resident') {
        const [rows] = await conn.query('SELECT id, amount, currency, status, created_at FROM payments WHERE user_id = ? ORDER BY created_at DESC', [req.user.id]);
        return res.json(rows);
      }
      // Authority: return all
      const [rows] = await conn.query('SELECT id, user_id, collector_id, amount, currency, status, created_at FROM payments ORDER BY created_at DESC');
      return res.json(rows);
    } finally { conn.release(); }
  } catch (err) {
    console.error(err);
