...
export default function RouteMap({ events }) {
    ...
    const validEvents = Array.isArray(events)
      ? events.filter(
          (ev) =>
            ev &&
            ev.location &&
            typeof ev.location.lat === "number" &&
            typeof ev.location.lng === "number"
        )
      : [];
    const defaultCenter =
  ...
  async function computeRoute() {
    // collect points
    const points = events
      .filter((e) => e.location && e.location.lat != null)
      .map((e) => ({ id: e.id, lat: e.location.lat, lng: e.location.lng }));
    ...
    <div className="bg-white p-4 rounded-lg">
      ...
      {sequence && sequence.length > 0 && (
        <div className="mt-2 text-sm text-gray-600">
          Route: {sequence.map((s, i) => `${i + 1}. ${s.id}`).join(" → ")}
        </div>
      )}
    </div>
  );
