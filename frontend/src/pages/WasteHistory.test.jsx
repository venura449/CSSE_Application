...
it('loads and displays schedules and requests lists (empty)', async () => {
  ...
  window.fetch.mockResolvedValueOnce({ ok: true, json: async () => ([])});
  render(<WasteHistory />);
  await waitFor(() => expect(screen.getAllByText(/Scheduled Collections/i)[0]).toBeInTheDocument());
  expect(screen.getAllByText(/No scheduled collections/i)[0]).toBeInTheDocument();
