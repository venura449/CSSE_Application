...
it('submits and stores token then navigates', async () => {
  window.fetch.mockResolvedValueOnce({
    ok: true,
    json: async () => ({ token: 't123', user: { email: 'u@e.com', role: 'Resident' } })
  });
  renderRoute();
  fireEvent.change(screen.getByPlaceholderText('Enter your email'), { target: { value: 'u@e.com' } });
...
it('shows error on failed signin', async () => {
  window.fetch.mockResolvedValueOnce({ ok: false, json: async () => ({ error: 'Invalid credentials' }) });
  renderRoute();
  fireEvent.change(screen.getByPlaceholderText('Enter your email'), { target: { value: 'u@e.com' } });
