...
export default function WasteHistory() {
  ...
  async function loadCollectors() {
    try {
      ...
      if (!token) return;
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:3000'}/api/collectors`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  async function assignCollector(requestId, collectorId) {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://127.0.0.1:3000'}/api/collections/request/${requestId}/assign`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ collector_id: collectorId }) });
      if (!res.ok) throw new Error(await res.text());
      toast.success('Collector assigned');
      await loadAll();
    } catch (err) { toast.error('Assign failed: ' + err.message); }
  }
  ...
  return (
    <div className="p-6">
      ...
      {user && user.role === 'Authority' && (() => {
        ...
        const scheds = (schedules || []).map(s => ({ id: s.id, kind: 'Schedule', type: s.type, date: s.date || s.scheduled_at || s.scheduledAt || null, status: s.status || 'Unknown', user_id: s.user_id || s.userId || null }));
        const reqs = (requests || []).map(r => ({ id: r.id, kind: 'Request', type: r.item_type || r.type || 'Special', date: r.preferred_datetime || r.created_at || null, status: r.status || 'Unknown', user_id: r.user_id || r.userId || null }));
        ...
        return (
          <section className="mb-6">
            ...
            {combined.length === 0 ? <p className="text-gray-500">No collections found.</p> : (
              <ul className="mt-2 space-y-2">
                {combined.map(c => (
                  <li key={`${c.kind}-${c.id}`} className="p-3 border rounded flex justify-between items-center">
                    <div>
                      <div className="font-medium">{c.kind}: {c.type}</div>
                      <div className="text-sm text-gray-600">{c.date ? new Date(c.date).toLocaleString() : 'No date'} — {c.status}</div>
                      <div className="text-xs text-gray-500">Owner ID: {c.user_id || 'Unknown'}</div>
                    </div>
                  </li>
                ))}
              </ul>
              ...
              <li key={s.id} className="p-3 border rounded flex justify-between items-center">
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    {(() => {
                      const dt = s.date || s.scheduled_at || s.scheduledAt || null;
                      try { return dt ? `${new Date(dt).toLocaleString()} — ${s.status || 'Unknown'}` : `${s.status || 'Unknown'}`; } catch (e) { return `${s.status || 'Unknown'}`; }
                    })()}
                  </div>
                  {/* show owner for Authority */}
                  {user && user.role === 'Authority' && s.user_id && (
                    <div className="text-xs text-gray-500">Owner ID: {s.user_id}</div>
                  )}
                </div>
              </li>
            ))}
              ...
              <li key={r.id} className="p-3 border rounded flex justify-between items-center">
                <div>
                  ...
                  <div className="text-sm text-gray-700">Estimated: ${Number(r.estimated_cost || 0).toFixed(2)}</div>
                    {user && user.role === 'Authority' && (
                      <div className="mt-2">
                        <label className="text-xs text-gray-500">Assign collector</label>
                        <select className="mt-1 border rounded p-1 text-sm" value={r.assigned_collector_id || ''} onChange={(e) => assignCollector(r.id, Number(e.target.value))}>
          ...
          <div className="bg-white w-full max-w-md rounded-xl p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <label className="block">
                ...
                <FieldHint>Example: Mattress, Electronics, Garden Waste</FieldHint>
              </label>
              <label className="block">
