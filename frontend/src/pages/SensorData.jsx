    ...
    <div className="bg-white rounded-lg shadow p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-gray-400">
          Last update: {new Date(node.updatedAt).toLocaleTimeString()}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-2">
        ...
        <div className="w-full h-2 bg-gray-200 rounded overflow-hidden">
          <div
            style={{ width: `${node.moist}%` }}
            className={`h-2 ${
              node.moist > 70 ? "bg-yellow-500" : "bg-green-400"
            }`}
          ></div>
        </div>
        ...
        <div className="w-full h-2 bg-gray-200 rounded overflow-hidden">
          <div
            style={{ width: `${node.smell}%` }}
            className={`h-2 ${node.smell > 70 ? "bg-red-400" : "bg-green-400"}`}
          ></div>
        </div>
      </div>
    </div>
    ...
    tickRef.current = setInterval(() => {
      setNodes((prev) =>
        prev.map((n, idx) => ({
          ...n,
          bin: randomWithin(
            n.bin + Math.round(Math.sin(Date.now() / 10000 + idx) * 2),
            idx
          ),
          moist: randomWithin(
            n.moist + Math.round(Math.cos(Date.now() / 12000 + idx) * 1),
            idx + 1
          ),
          smell: randomWithin(
            n.smell + Math.round(Math.sin(Date.now() / 8000 + idx * 0.3) * 1),
            idx + 2
          ),
          updatedAt: Date.now(),
        }))
        ...
        prev.forEach((n) => {
          if (n.bin >= THRESHOLD) {
            if (!alertedRef.current.has(n.id)) {
              ...
              toast.warning(`${n.id} (${n.road}) bin level high: ${n.bin}%`, {
                duration: 8000,
              });
            }
          } else {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-sm text-gray-500">
            Live demo — values update every 10s
          </div>
        </div>
