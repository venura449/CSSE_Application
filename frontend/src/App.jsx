...
function App() {
  ...
  const invalid = importsToCheck.filter(
    (i) =>
      !(i.comp && (typeof i.comp === "function" || typeof i.comp === "object"))
  );
  if (
    invalid.length > 0 &&
    import.meta &&
    ...
    return (
      <div style={{ padding: 24 }}>
        <h2 style={{ color: "#b91c1c" }}>
          Import diagnostic: invalid component(s)
        </h2>
        <p>
          The app detected one or more imported components are undefined or not
          valid React components.
        </p>
        <ul>
          {invalid.map((i) => (
            <li key={i.name}>
              <strong>{i.name}</strong> — value: {String(i.comp)}
            </li>
          ))}
        </ul>
        <p>
          Check your imports/exports for those modules. Open the browser console
          for more details.
        </p>
      </div>
    );
